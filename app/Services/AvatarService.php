...
class AvatarService
{
    ...
    public function uploadToCloud($file){
      ...
      $avatar = $this->avatar->create(['filename' => $filename, 'user_id' => \auth()->id()]); //TODO User class
      ProcessImageResize::dispatch($avatar, $this->sizes);
      return $avatar;
    }

    public function getByUserId(Avatar $avatarModel, $userId){
      $avatar = $avatarModel->whereUserId($userId)->first();
